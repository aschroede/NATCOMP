...
class Particle {
	...
	alignmentVector( neighborRadius ){
		...
		if (neighbors.length == 0){ return this.dir }
	...
	cohesionVector( neighborRadius ){
		...
		return this.normalizeVector(this.subtractVectors(avg, this.pos))
...
const metricsPath = getInput( "M", undefined, "string" ) // path to write metrics to
...
class Scene {
	...
	getNearestNeighbourDistances(){
		
		let nearestNeighbourDistances = []
		for( let p of this.swarm ){
			let nearest = undefined
			let nearestDist = 1000000
			for( let q of this.swarm ){
				if( p.id == q.id ) continue
				let dist = this.dist( p.pos, q.pos )
				if( dist < nearestDist ){
					nearest = q
					nearestDist = dist
				}
			}
			nearestNeighbourDistances.push( nearestDist )
		}
		return nearestNeighbourDistances
	}
	...
	getOrderParameter(){
		let normSum = [0,0]
		for( let p of this.swarm ){
			normSum = this.addVectors(normSum, this.normalizeVector(p.dir))
		}

		// Returns a single value
		let norm = this.getEuclideanNorm(normSum)

		// Divide by total number of particles
		return norm/this.swarm.length
	}
	...
	// This computes v/||v||, where ||v|| is the Euclidean norm of v
	normalizeVector( a ){
	
		if( a[0] == 0 & a[1] == 0 ) return [0,0]
	
		let norm = this.getEuclideanNorm(a)
		for( let i = 0 ; i < a.length ; i ++ ){
			a[i] /= norm
		}
		return a
...
if ( collectMetrics ){
	fs.writeFileSync(`${metricsPath}/orderParameter.csv`, "time,orderParameter\n")
	fs.writeFileSync(`${metricsPath}/nearestNeighborParameter.csv`, "time,nearestNeighborParameter\n")
}
...
for( let t = 0; t <= conf.runTime; t++ ){
	...
	// Save the OrderParameter 

	if ( collectMetrics & t % 10 == 0 ){
		const op = S.getOrderParameter()
